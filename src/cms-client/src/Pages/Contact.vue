...
const submit = () => {
    form.name = DOMPurify.sanitize($('input#name').val());
    form.email = DOMPurify.sanitize($('input#email').val());
    form.message = DOMPurify.sanitize($('textarea#message').val());
    form.transform(data => ({
        ...data,
...
<template>
   <AppLayout title="Contact" :appname="appname">

    <Head title="Contact" />
    <section>
      <div class="max-w-screen-xl mt-2 px-8 grid gap-8 grid-cols-1 md:grid-cols-2 md:px-12 lg:px-16 xl:px-32 py-16 mx-auto bg-gray-100 text-gray-900 rounded-lg">
        <div class="flex flex-col justify-between">
          <div>
            <h2 class="text-4xl lg:text-5xl font-bold leading-tight">Entre em contato conosco</h2>
            <div class="text-gray-700 mt-8">
              Certifique-se de informar um email valido para obter resposta.
            </div>
          </div>
		...
		<div class="mt-0">
          ...
          <div class="mt-8">
            <JetLabel class="uppercase text-sm text-gray-600 font-bold" value="Mensagem" />
            <textarea class="mt-1 block w-full" id="message" v-model="form.message" required></textarea>
          </div>
          <div class="mt-8">

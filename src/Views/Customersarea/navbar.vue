...
const text_theming = props.customersarea.themecolor || 'black';
...
<style scoped>
.text-theming, .text-theming:active{
color: v-bind(text_theming);
fill: v-bind(text_theming);
}
</style>
    ...
    <div class="container flex items-center justify-between h-full px-6 mx-auto text-purple-600 dark:text-purple-300">
      ...
      <div class="flex justify-center flex-1 lg:mr-32">
        <div class="relative w-full max-w-xl mr-6 focus-within:text-purple-500">
          ...
          <form v-if="customersarea.search">
            <input ref="search"
              class="w-full pl-8 pr-2 text-sm text-gray-700 placeholder-gray-600 bg-gray-100 border-0 rounded-md dark:placeholder-gray-500 dark:focus:shadow-outline-gray dark:focus:placeholder-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:placeholder-gray-500 focus:bg-white focus:border-purple-300 focus:outline-none focus:shadow-outline-purple form-input"
              type="text" placeholder="Search for posts" aria-label="Search" v-model="searchItem" />
          </form>
        </div>
      </div>
      <ul class="flex items-center flex-shrink-0 space-x-6">
        <!-- Theme toggler -->
        <li v-if="customersarea.darkmode" class="flex">
          <button class="rounded-md focus:outline-none focus:shadow-outline-purple" v-on:click="toggleTheme"
            aria-label="Toggle color mode">
            <template v-if="!dark">
              <svg class="w-5 h-5 text-theming" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
        ...
        <!-- Notifications menu -->
        <li v-if="customersarea.notifications" class="relative">
          ...
          <template v-if="isNotificationsMenuOpen">
            <ul x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
              ...
              aria-label="submenu">
              <li class="flex">
                <Link class="inline-flex items-center justify-between w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200"
                  :href="route('dashboard.notifications') + '?tab=mynotifications'">
                  <span v-if="lang == 'pt_BR'">Mensagens</span>
                  <span v-else-if="lang == 'en_US'">Messages</span>
              ...
              <li class="flex">
                <a class="inline-flex items-center justify-between w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200"
                  :href="route('dashboard.notifications') + '?tab=notifications'">
                  <span v-if="lang == 'pt_BR'">Todas as notificações</span>
                  <span v-else-if="lang == 'en_US'">All notifications</span>
            ...
            <ul x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
              ...
              <li class="flex">
                <Link class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200"
                  ...
                  <span v-if="lang == 'pt_BR'">Perfil</span>
                  <span v-else-if="lang == 'en_US'">Profile</span>
              ...
              <li class="flex">
                <Link class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200"
                  ...
                  <span v-if="lang == 'pt_BR'">Histórico</span>
                  <span v-else-if="lang == 'en_US'">History</span>
              ...
              <li class="flex">
                <button class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200"
                  ...
                  <span v-if="lang == 'pt_BR'">Desconectar</span>
                  <span v-else-if="lang == 'en_US'">Log out</span>

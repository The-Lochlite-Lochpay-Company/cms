...
const form = useForm({
    name: props.showroute.name,
    type: props.showroute.type,
    url: props.showroute.url,
    controller: props.showroute.controller,
    action: props.showroute.action,
    middleware: props.showroute.middleware,
    status: props.showroute.status,
    only: props.showroute.only,
    except: props.showroute.except,
});
const submit = (event) => {
    form.put(route('managerroutes.update', {id: event.submitter.dataset.route}));
};
</script>
<template>
<AppLayout :menuitems="menuitems" :avatar="avatar" :role="role" :name="name" :version="version" :breadcrumbCurrentTitle="breadcrumbCurrentTitle" :breadcrumbCurrentSection="breadcrumbCurrentSection" :title="title">
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 col-md-12 text-center">
                <JetButton type="submit" class="btn btn-primary" title="Atualizar" aria-label="Atualizar" :class="{ 'opacity-25': form.processing }" :disabled="form.processing" data-event="submit" data-function="update" :data-route="showroute.id">Atualizar</JetButton>
            </div>
        </div>
      </form>
